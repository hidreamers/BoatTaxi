...
class AdvertisementRepository @Inject constructor(
    ...
    suspend fun getUserAdvertisements(): List<Advertisement> {
        ...
        val ads = firestore.collection("advertisements")
        ...
        android.util.Log.d("AdRepo", "Found ${ads.size} ads for user. IDs: ${ads.map { it.id }}")
        ads.forEach { ad ->
            android.util.Log.d("AdRepo", "  Ad: ${ad.businessName}, status=${ad.status}, id=${ad.id}, imageUrl=${ad.imageUrl}, youtubeUrl=${ad.youtubeUrl}")
        }
    ...
    suspend fun createDemoAds(description: String = "", count: Int = 10): Result<Int> = runCatching {
        ...
        for (i in 0 until actualCount) {
            ...
            val adMap = hashMapOf<String, Any?>(
                ...
                "status" to AdStatus.ACTIVE.name,
                "isFeatured" to (i < 3), // First 3 are featured
                "impressions" to (50..500).random(),
                "clicks" to (5..50).random(),
                "price" to 0.0, // AI generated = free
                "paymentStatus" to PaymentStatus.COMPLETED.name,
                "startDate" to now,
                "endDate" to Timestamp(now.seconds + 14 * 24 * 60 * 60, 0), // 14 days
                "createdAt" to now,
                "updatedAt" to now,
                "hasCoupon" to true,
                ...
                "couponExpiresAt" to Timestamp(now.seconds + 14 * 24 * 60 * 60, 0),
